<script>
  ...
  function handleDrop(event, targetConcept) {
    event.preventDefault();
    if (draggingItem && draggingItem.name !== targetConcept.name && !targetConcept.archived) {
        playChoiceClick();
  ...
  $: displayInventory = showArchived ? [...activeInventory, ...archivedInventory] : activeInventory;
</script>
...
<style>
    ...
    .concept-type[data-kind="sensorial"] { color: #e67e22; }
    .concept-type[data-kind="deducido"] { color: #2ecc71; }
    .concept-type[data-kind="falso"] { color: #e74c3c; }
